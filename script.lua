...
-- ============================================================
--  SUITE V9.27 - OPSYX  (sUNC 100% / UNC 98-99% HARDENED)
-- ============================================================
--  V9.27 CHANGES on top of V9.26:
--
--  [FIX-IG1]  IGNORE LIST now sorts by proximity to local
--             player. Closest player is always first in the
--             list. Distance badge shown on each row (e.g.
--             "12 m"). List re-sorts live every refresh tick
--             (~0.25s) so it stays accurate as players move.
--             Players with no loaded character (distance
--             unknown) fall to the bottom, sorted by name.
--
--  [FIX-IG2]  Added a "SORT: NEAR" / "SORT: A-Z" toggle
--             button inside the ignore panel header so the
--             user can switch between proximity sort and
--             alphabetical sort without leaving the panel.
--
--  [FIX-IG3]  Ignored players now show their distance too,
--             so the user can tell who is nearby even when
--             they're already on the list.
--
--  [FIX-IG4]  Ignore panel now auto-refreshes every 0.25s
--             while it is open (distance updates live).
--             Previously it only refreshed on open or player
--             join/leave.
--
--  [IMPROVE]  Ignore panel row height increased to 30px and
--             distance badge added on the right side of each
--             row in a muted color so it doesn't clutter the
--             name.
--
--  [IMPROVE]  Panel header shows player count and "X ignored"
--             count so the user always sees the state at a
--             glance without opening the list.
--
--  All V9.26 + V9.25 fixes retained.
-- ============================================================
...
local tdf = (task and type(task.defer) == "function") and task.defer
    or tsp
...
local function applyFPSUnlock()
    pcall(function() if type(setfpscap)    == "function" then setfpscap(FPS_TARGET) end end)
    pcall(function() if type(fps_unlock)   == "function" then fps_unlock(FPS_TARGET == 0 and 9999 or FPS_TARGET) end end)
    pcall(function() if syn and type(syn.fps_unlock) == "function" then syn.fps_unlock(FPS_TARGET == 0 and 9999 or FPS_TARGET) end end)
    pcall(function() if Drawing and type(Drawing.setfpscap) == "function" then Drawing.setfpscap(FPS_TARGET) end end)
    pcall(function() if type(executor_settings) == "table" then executor_settings.fps_cap = FPS_TARGET == 0 and 9999 or FPS_TARGET end end)
end

applyFPSUnlock()
local function reapplyFPS() tw(0.5); applyFPSUnlock() end
...
local function rn(l)
    ...
    local p    = #pool
    local t    = {}
    for i = 1, l or 8 do local c = math.random(1,p); t[i] = pool:sub(c,c) end
    return table.concat(t)
end
...
-- ============================================================
-- COLOR CONSTANTS  [FIX-KB1 retained]
-- ============================================================
...
-- SETTINGS
-- ============================================================
local ESP_MAX_RANGE     = 2000
local ESP_STEALTH_RANGE = 60
local LOCK_MARGIN       = 45

local S = {
    KB = {am="F1", es="F2", sl="F3", tr="F4", wc="F6"},
    AM = {on=false, sm=0.35, md=1000, pd=0.05, tc=true, wc=false, lo=0.15},
    SL = {on=false, sm=0.3,  md=1000, tc=true, wc=false, pd=0.05, sp=0.06},
    TR = {on=false, dl=0.05, md=1000, rd=true, tc=true,  wc=false, hr=0.09},
    ES = {on=false, md=ESP_MAX_RANGE, sd=ESP_STEALTH_RANGE, tc=true,
...
local ST = {
    ...
    igSortNear=true,   -- [FIX-IG2] true=proximity sort, false=alpha sort
    igLastRefresh=0,   -- [FIX-IG4] timestamp of last auto-refresh
    fr=0, espT=0,
    hid=false,
    stealth=false, kills=0, killT=0, lkT=0,
    arm=false, saArm=false, _rb=nil,
    holdReleased=false,
}

-- ============================================================
-- PILLS registry [FIX-KB5 retained]
-- ============================================================
local PILLS = {}
...
local function mk(input, key)
    ...
    local ok, tc = pcall(function() return target.EnumType end)
    if not ok then return false end
    if tc == Enum.KeyCode then
        return input.KeyCode == target
    else
...
local function clearPartCache() PART_CACHE = {}; LOSC = {} end

local function clearLOSCForChar(c)
    if not c then return end
    for part in pairs(LOSC) do
        if part and part.Parent == c then LOSC[part] = nil end
    end
end
...
-- ============================================================
-- [FIX-IG1] PROXIMITY HELPER
-- Returns distance from local player's root to another
-- player's root. Returns math.huge if either has no character.
-- ============================================================
local function distToPlayer(pl)
    local myChar = ME.Character
    if not myChar then return math.huge end
    local myRoot = fr(myChar)
    if not myRoot then return math.huge end
    local theirChar = pl.Character
    if not theirChar then return math.huge end
    local theirRoot = fr(theirChar)
...
local function doScan()
    ...
    local sorted = {}
    local lockedFirst = nil
    for i = 1, #all do
        if all[i] == ST.tgpl then lockedFirst = all[i]
        else sorted[#sorted+1] = all[i] end
    end
    if lockedFirst then table.insert(sorted, 1, lockedFirst) end
        ...
        if ck(pl) then
            ...
            if p then
                ...
                if on then
                    ...
                    e.mD = useCen and e.cD or math.sqrt((sp.X-mx)^2 + (sp.Y-my)^2)
...
local function findTarget(fov, md, tc, wc, useCenter)
    ...
    local switchThr  = lockedDist - LOCK_MARGIN

    local bestPart, bestPl, bestDist = nil, nil, fovLimit
        ...
        if d < fovLimit and e.md <= (md or 1000) then
            ...
            if okT and not (wc and not los(e.p)) then
                if e.pl == ST.tgpl then
                    bestPart = e.p; bestPl = e.pl; bestDist = d
                    break
                elseif d < switchThr and d < bestDist then
                    bestDist = d; bestPart = e.p; bestPl = e.pl
                end
            end
        end
    end

    ST.tgDist = bestPl ~= nil and bestDist or 999
...
local function doAimbot(dt)
    if ST.holdReleased then
        ST.holdReleased = false; flushTarget(); return
    end
    if not S.AM.on then flushTarget(); return end

    local p, pl = findTarget(S.FV.r, S.AM.md, S.AM.tc, S.AM.wc, false)
    if not p then flushTarget(); return end
    ...
    if not haveMouse() then
        ...
        local sm    = S.AM.sm * (1 - dNorm*0.6)
        if ramping   then sm = sm * 0.55 end
        if switching then sm = sm * 0.4  end
        sm = math.max(0.01, sm)
        local al2 = 1 - math.pow(1 - sm, (dt or 0.016)*60)
        if ramping then
    ...
    if ramping then
        local js = cl(d/120, 0.02, 0.6)
        mx = mx + RNG:NextNumber(-0.5,0.5) * js
        my = my + RNG:NextNumber(-0.5,0.5) * js
    end

    local mm = (10 + dNorm*30) * cl((dt or 0.016)*60, 0.5, 2.0)
    if math.abs(mx) > mm then mx = sg(dx)*mm end
    if math.abs(my) > mm then my = sg(dy)*mm end

    if now - ST.skipT > 0.8 then
        if RNG:NextNumber() < 0.02 then ST.skipT = now; return end
    end
...
local function hpColor(hum)
    ...
    if pct > 0.6 then
        local t = (pct - 0.6) / 0.4
        return Color3.fromRGB(math.floor((1-t)*255), 180, 0)
    else
        local t = pct / 0.6
        return Color3.fromRGB(200, math.floor(t*180), 0)
    end
end
...
local function createInstanceESP(pl)
    local ok = pcall(function()
        ...
        hl.Name = hlN; hl.FillTransparency = 0.5; hl.OutlineTransparency = 0.2
        hl.FillColor = isEnemy(pl) and S.ES.ce or S.ES.ct
        hl.OutlineColor = Color3.fromRGB(255,255,255)
        hl.Adornee = c; hl.Parent = c

        local bg = Instance.new("BillboardGui")
        bg.Name = bgN; bg.Size = UDim2.new(0,200,0,50)
        ...
        t1.Size = UDim2.new(1,0,0,26); t1.BackgroundTransparency = 1
        t1.Text = pl.Name; t1.TextColor3 = Color3.fromRGB(255,255,255)
        t1.TextStrokeTransparency = 0.2; t1.TextStrokeColor3 = Color3.fromRGB(0,0,0)
        t1.Font = Enum.Font.GothamBold; t1.TextSize = 16; t1.Parent = bg

        local t2 = Instance.new("TextLabel")
        t2.Size = UDim2.new(1,0,0,18); t2.Position = UDim2.new(0,0,0,28)
        t2.BackgroundTransparency = 1; t2.Text = ""
        t2.TextColor3 = Color3.fromRGB(180,220,180)
        t2.TextStrokeTransparency = 0.4; t2.TextStrokeColor3 = Color3.fromRGB(0,0,0)
        t2.Font = Enum.Font.Gotham; t2.TextSize = 11; t2.Parent = bg
...
local function setMenuVisible(v)
    ...
    if not v then
        ...
        ST.mn = false; ST.igOpen = false
        SI.dragging = false; ST._rb = nil
...
local function toggleKeysPanel()
    ...
    ST.mn = not ST.mn; GUI.setPanel.Visible = ST.mn
end
...
-- ============================================================
-- IGNORE PANEL REFRESH  [FIX-IG1][FIX-IG2][FIX-IG3][FIX-IG4]
-- ============================================================
local function refreshIgnorePanel()
    ...
    -- Build candidate list with live distances
    local list = {}
    local ignoredCount = 0
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl ~= ME then
            if q == "" or pl.Name:lower():find(q, 1, true) then
                local dist = distToPlayer(pl)  -- [FIX-IG1]
                list[#list+1] = {pl=pl, dist=dist}
            end
            if isIgnored(pl) then ignoredCount = ignoredCount + 1 end
        end
    end

    -- [FIX-IG2] Sort: proximity (nearest first) OR alphabetical
    if ST.igSortNear then
        -- Near-first: unknown distance (math.huge) falls to bottom
        -- Among same-distance players (both have no char), sort by name
    ...
    -- Update header status label: "12 players  |  3 ignored"
    if GUI.igStatusLbl then
        local total = #list
        GUI.igStatusLbl.Text = total .. " players  |  " .. ignoredCount .. " ignored"
    end

    -- Update sort button label to reflect current mode
    ...
    local ROW     = 32   -- [IMPROVE] taller rows
    local contH   = math.max(60,
        container.AbsoluteSize.Y > 0
        and container.AbsoluteSize.Y
        or container.Size.Y.Offset)
    ...
    for i = 1 + ST.igScroll, math.min(#list, ST.igScroll + VISIBLE) do
        ...
        row.BackgroundColor3 = ignored
            and Color3.fromRGB(110,25,25)
            or  Color3.fromRGB(32,32,52)
        ...
        -- Toggle button (click anywhere on the row)
        ...
        nameLbl.TextColor3 = ignored
            and Color3.fromRGB(255,110,110)
        ...
        -- [FIX-IG3] Distance badge on the right
        local distLbl = Instance.new("TextLabel")
        distLbl.Size = UDim2.new(0,56,1,0)
        distLbl.Position = UDim2.new(1,-58,0,0)
        distLbl.BackgroundTransparency = 1
        local distStr
        if dist == math.huge then
            distStr = "? m"
        elseif dist >= 1000 then
            distStr = math.floor(dist/100)/10 .. "km"
        else
            distStr = math.floor(dist+0.5) .. " m"
        end
        distLbl.Text = distStr
        -- Color: green<50m, yellow<150m, red further, gray=unknown
        if dist == math.huge then
            distLbl.TextColor3 = Color3.fromRGB(100,100,120)
        elseif dist < 50 then
            distLbl.TextColor3 = Color3.fromRGB(80,220,80)
        elseif dist < 150 then
            distLbl.TextColor3 = Color3.fromRGB(220,180,60)
        else
            distLbl.TextColor3 = Color3.fromRGB(180,100,100)
        end
        distLbl.TextSize = 11
        ...
        clickBtn.MouseEnter:Connect(function()
            row.BackgroundColor3 = isIgnored(cap)
                and Color3.fromRGB(160,45,45)
                or  Color3.fromRGB(55,55,80)
        end)
    ...
    if GUI.igUp then
        GUI.igUp.BackgroundColor3 = ST.igScroll > 0
            and Color3.fromRGB(60,40,90)
            or  Color3.fromRGB(25,22,35)
        GUI.igUp.TextColor3 = ST.igScroll > 0
            and Color3.fromRGB(220,180,255)
            or  Color3.fromRGB(70,60,90)
    end
    if GUI.igDown then
...
local function cg()
    ...
    screenGui.Name = gn; screenGui.ResetOnSpawn = false
    ...
    if not ok then print("[V9.27-OPSYX] No GUI parent"); return end
    GUI.sg = screenGui
    ...
    main.Name = mn2; main.Size = UDim2.new(0,220,0,320)
    ...
    accent.Size = UDim2.new(1,0,0,2); accent.Position = UDim2.new(0,0,0,26)
    accent.BackgroundColor3 = Color3.fromRGB(0,180,255)
    accent.BorderSizePixel = 0; accent.Parent = main

    local titleLbl = Instance.new("TextLabel")
    titleLbl.Size = UDim2.new(1,-56,0,26); titleLbl.Position = UDim2.new(0,10,0,0)
    titleLbl.BackgroundTransparency = 1
    titleLbl.Text = "OPSYX v9.27  |  -- FPS"
    ...
    local function makeWBtn(xOff, col, lbl)
        ...
        b.TextSize = 12; b.Font = Enum.Font.GothamBold; b.Parent = main
    ...
    restoreBar.Size = UDim2.new(0,100,0,22); restoreBar.Position = UDim2.new(0,10,0,40)
    restoreBar.BackgroundColor3 = Color3.fromRGB(10,10,18)
    restoreBar.BackgroundTransparency = 0.08; restoreBar.BorderSizePixel = 0
    restoreBar.Text = "OPSYX"; restoreBar.TextColor3 = Color3.fromRGB(0,200,255)
    ...
    local function makeToggle(lbl, yPos, offCol, onCol, getter, setter)
        local row = Instance.new("Frame")
        row.Size = UDim2.new(0.92,0,0,26); row.Position = UDim2.new(0.04,0,0,yPos)
        row.BackgroundColor3 = Color3.fromRGB(18,18,30)
        row.BackgroundTransparency = 0.3; row.BorderSizePixel = 0; row.Parent = main
        pcall(function() Instance.new("UICorner",row).CornerRadius = UDim.new(0,6) end)

        local lbEl = Instance.new("TextLabel")
        lbEl.Size = UDim2.new(0.6,0,1,0); lbEl.BackgroundTransparency = 1
        lbEl.Text = lbl; lbEl.TextColor3 = Color3.fromRGB(210,210,220)
        lbEl.TextSize = 12; lbEl.Font = Enum.Font.GothamBold
        lbEl.TextXAlignment = Enum.TextXAlignment.Left
        lbEl.Position = UDim2.new(0,8,0,0); lbEl.Parent = row

        local pill = Instance.new("TextButton")
        pill.Size = UDim2.new(0,46,0,18); pill.Position = UDim2.new(1,-52,0.5,-9)
        pill.BackgroundColor3 = getter() and onCol or offCol
        pill.BorderSizePixel = 0; pill.Text = getter() and "ON" or "OFF"
        pill.TextColor3 = Color3.fromRGB(255,255,255)
        pill.TextSize = 10; pill.Font = Enum.Font.GothamBold; pill.Parent = row
    ...
    makeToggle("AIMBOT",    Y, C_RED, C_GRN,
        function() return S.AM.on end,
        function() S.AM.on=not S.AM.on; if not S.AM.on then flushTarget() end end)
    Y = Y + GAP

    makeToggle("ESP",       Y, C_RED, C_GRN,
        function() return S.ES.on end,
        function() S.ES.on=not S.ES.on; if not S.ES.on then destroyAllInstanceESP() end end)
    Y = Y + GAP

    makeToggle("SILENT",    Y, C_ORG, C_GRN,
        function() return S.SL.on end,
        function() S.SL.on=not S.SL.on end)
    Y = Y + GAP

    makeToggle("TRIGGER",   Y, C_RED, C_GRN,
        function() return S.TR.on end,
        function() S.TR.on=not S.TR.on end)
    Y = Y + GAP

    makeToggle("FOV CIRCLE",Y, C_BLU, C_GRN,
        function() return S.FV.on end,
        function() S.FV.on=not S.FV.on end)
    Y = Y + GAP

    makeToggle("WALL CHECK",Y, C_ORG, C_GRN,
        function() return S.AM.wc end,
        function() local nv=not S.AM.wc; S.AM.wc=nv; S.SL.wc=nv; S.TR.wc=nv end)
    Y = Y + GAP

    makeToggle("HOLD AIM",  Y, C_ORG, C_GRN,
        function() return holdToAimEnabled end,
        function()
            holdToAimEnabled = not holdToAimEnabled
            if not holdToAimEnabled then
                S.AM.on=false; ST.arm=false; ST.saArm=false; flushTarget()
            end
        end)
    Y = Y + GAP

    local igBtn = Instance.new("TextButton")
    igBtn.Size = UDim2.new(0.92,0,0,26); igBtn.Position = UDim2.new(0.04,0,0,Y)
    igBtn.BackgroundColor3 = Color3.fromRGB(55,20,65); igBtn.BackgroundTransparency = 0.2
    igBtn.BorderSizePixel = 0; igBtn.Text = "IGNORE LIST"
    igBtn.TextColor3 = Color3.fromRGB(200,140,255)
    igBtn.TextSize = 12; igBtn.Font = Enum.Font.GothamBold; igBtn.Parent = main
    pcall(function() Instance.new("UICorner",igBtn).CornerRadius = UDim.new(0,6) end)
    Y = Y + GAP

    local setBtn = Instance.new("TextButton")
    setBtn.Size = UDim2.new(0.92,0,0,26); setBtn.Position = UDim2.new(0.04,0,0,Y)
    setBtn.BackgroundColor3 = Color3.fromRGB(25,25,70); setBtn.BackgroundTransparency = 0.2
    setBtn.BorderSizePixel = 0; setBtn.Text = "KEYS  [F5]"
    setBtn.TextColor3 = Color3.fromRGB(150,180,255)
    setBtn.TextSize = 12; setBtn.Font = Enum.Font.GothamBold; setBtn.Parent = main
    pcall(function() Instance.new("UICorner",setBtn).CornerRadius = UDim.new(0,6) end)

    -- ================================================================
    -- IGNORE PANEL  [FIX-IG1][FIX-IG2][FIX-IG3][FIX-IG4]
    -- ================================================================
    local igPanel = Instance.new("Frame")
    igPanel.Name = bn; igPanel.Size = UDim2.new(0,246,0,520)
    igPanel.Position = UDim2.new(1,-284,0,70)
    igPanel.BackgroundColor3 = Color3.fromRGB(10,10,18)
    igPanel.BackgroundTransparency = 0.08; igPanel.BorderSizePixel = 0
    igPanel.Active = true; igPanel.Draggable = true
    ...
    -- [IMPROVE] Status line: player count + ignored count
    ...
    igStatusLbl.TextColor3 = Color3.fromRGB(130,130,160)
    ...
    GUI.igStatusLbl = igStatusLbl

    -- Search box
    local igSearch = Instance.new("TextBox")
    igSearch.Size = UDim2.new(1,-10,0,24); igSearch.Position = UDim2.new(0,5,0,44)
    igSearch.BackgroundColor3 = Color3.fromRGB(20,20,34); igSearch.BackgroundTransparency = 0.2
    igSearch.BorderSizePixel = 0; igSearch.PlaceholderText = "Search player..."
    igSearch.Text = ""; igSearch.TextColor3 = Color3.fromRGB(220,220,255)
    igSearch.PlaceholderColor3 = Color3.fromRGB(100,100,135)
    ...
    GUI.igSortBtn = igSortBtn

    igSortBtn.MouseButton1Down:Connect(function()
        ST.igSortNear = not ST.igSortNear
        pcall(refreshIgnorePanel)
    end)
    igSortBtn.MouseEnter:Connect(function()
        igSortBtn.BackgroundColor3 = Color3.fromRGB(65,40,95)
    end)
    igSortBtn.MouseLeave:Connect(function()
        igSortBtn.BackgroundColor3 = Color3.fromRGB(40,25,60)
    end)

    -- Container for player rows
    local igContainer = Instance.new("Frame")
    igContainer.Size = UDim2.new(1,-10,0,356)
    igContainer.Position = UDim2.new(0,5,0,95)
    igContainer.BackgroundTransparency = 1; igContainer.BorderSizePixel = 0
    igContainer.ClipsDescendants = true
    igContainer.Parent = igPanel
    ...
    igUp.BackgroundColor3 = Color3.fromRGB(25,22,35); igUp.BackgroundTransparency = 0.2
    igUp.BorderSizePixel = 0; igUp.Text = "▲  UP"
    igUp.TextColor3 = Color3.fromRGB(70,60,90); igUp.TextSize = 11
    ...
    igUp.MouseButton1Down:Connect(function()
        ST.igScroll = math.max(0,(ST.igScroll or 0)-5)
        pcall(refreshIgnorePanel)
    end)
    GUI.igUp = igUp
    ...
    igDown.BackgroundColor3 = Color3.fromRGB(25,22,35); igDown.BackgroundTransparency = 0.2
    igDown.BorderSizePixel = 0; igDown.Text = "▼  DOWN"
    igDown.TextColor3 = Color3.fromRGB(70,60,90); igDown.TextSize = 11
    ...
    igDown.MouseButton1Down:Connect(function()
        ST.igScroll = (ST.igScroll or 0) + 5
        pcall(refreshIgnorePanel)
    end)
    GUI.igDown = igDown
    ...
    igClearBtn.Size = UDim2.new(0,60,0,26); igClearBtn.Position = UDim2.new(1,-70,1,-60)
    igClearBtn.BackgroundColor3 = Color3.fromRGB(70,20,20); igClearBtn.BackgroundTransparency = 0.2
    igClearBtn.BorderSizePixel = 0; igClearBtn.Text = "CLEAR"
    igClearBtn.TextColor3 = Color3.fromRGB(255,130,130); igClearBtn.TextSize = 10
    ...
    igClearBtn.MouseButton1Down:Connect(function()
        for pl in pairs(IGNORE) do IGNORE[pl] = nil end
        pcall(refreshIgnorePanel)
    end)

    local igClose = Instance.new("TextButton")
    igClose.Size = UDim2.new(0,60,0,26); igClose.Position = UDim2.new(0.5,-30,1,-28)
    igClose.BackgroundColor3 = Color3.fromRGB(50,18,18); igClose.BackgroundTransparency = 0.2
    igClose.BorderSizePixel = 0; igClose.Text = "CLOSE"
    igClose.TextColor3 = Color3.fromRGB(255,140,140); igClose.TextSize = 11
    ...
    igClose.MouseButton1Down:Connect(function()
        igPanel.Visible = false; ST.igOpen = false
    end)

    igBtn.MouseButton1Down:Connect(function()
        ST.igOpen = not ST.igOpen
        igPanel.Visible = ST.igOpen
        if ST.igOpen then tdf(function() pcall(refreshIgnorePanel) end) end
    end)
    ...
    setP.Name = sn; setP.Size = UDim2.new(0,350,0,400)
    setP.Position = UDim2.new(0,240,0,70)
    setP.BackgroundColor3 = Color3.fromRGB(10,10,18)
    setP.BackgroundTransparency = 0.08; setP.BorderSizePixel = 0
    setP.Active = true; setP.Draggable = true
    ...
    spTitle.TextSize = 12; spTitle.Font = Enum.Font.GothamBold; spTitle.Parent = setP
    ...
    for i = 1, #binds do
        ...
        lbl2.Size = UDim2.new(0,150,0,26); lbl2.Position = UDim2.new(0,10,0,ry)
        lbl2.BackgroundTransparency = 1; lbl2.Text = bd.l
        lbl2.TextColor3 = Color3.fromRGB(190,190,200)
        ...
        kb.MouseButton1Down:Connect(function()
            ...
            kb.Text = "[ press key ]"
            kb.TextColor3 = Color3.fromRGB(255,200,0)
        end)
    end
    GUI.kbBtns = KB_BTNS
    ...
    local function fixedRow(label, keyName, ry)
        ...
        lbl2.TextColor3 = Color3.fromRGB(160,160,170)
    ...
    sBtn.Size = UDim2.new(0,16,0,16); sBtn.Position = UDim2.new(S.FV.r/500,-8,0,-5)
    ...
    sBg.MouseLeave:Connect(function() SI.dragging = false end)
end
...
local okIn, errIn = pcall(function()
    hook(UI.InputBegan:Connect(function(inp, gpd)
        ...
        if ST._rb then
            if isMouseBtn(inp) then
                if inp.UserInputType == Enum.UserInputType.MouseButton2 then
                    local nn = "MouseButton2"
                    S.KB[ST._rb.key]      = nn
                    ST._rb.btn.Text       = nn
                    ST._rb.btn.TextColor3 = Color3.fromRGB(0,200,255)
                    if GUI.kbBtns and GUI.kbBtns[ST._rb.key] then
                        GUI.kbBtns[ST._rb.key].Text = nn
                    end
                    ST._rb = nil
                end
                return
            end
            if inp.KeyCode == Enum.KeyCode.F5 or inp.KeyCode == Enum.KeyCode.F7 then
        ...
        -- RMB arm  (even when gpd=true)
        if inp.UserInputType == Enum.UserInputType.MouseButton2 then
            ST.arm = true; ST.saArm = true
        ...
        -- F7: toggle menu only, FOV unaffected
        if inp.KeyCode == Enum.KeyCode.F7 then
            if GUI.main then
                local nv = not GUI.main.Visible
                GUI.main.Visible = nv
                ST.hid = not nv
                if GUI.restoreBar then GUI.restoreBar.Visible = not nv end
                if not nv then
                    if GUI.igPanel  then GUI.igPanel.Visible  = false end
                    if GUI.setPanel then GUI.setPanel.Visible = false end
                    ST.mn = false; ST.igOpen = false
        ...
        if mk(inp,"am") then
            S.AM.on = not S.AM.on
            if not S.AM.on then flushTarget() end
        ...
        if mk(inp,"wc") then
            ...
            updBtn("WALL CHECK",nv,      C_GRN, C_ORG)
        end
    end))

    hook(UI.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton2 then
            ST.arm = false; ST.saArm = false
...
if MOB then
    pcall(function()
        ...
        local function mt(l, y, gs, fn, armSA)
            ...
            b.BackgroundColor3 = gs() and C_GRN or C_RED
...
local okLoop, errLoop = pcall(function()
    conn = RS.RenderStepped:Connect(function(dt)
        ...
        pcall(function()
            ...
            if elapsed >= FPS_INTERVAL then
                ...
                if GUI.titleLabel then
                    GUI.titleLabel.Text = "OPSYX v9.27  |  " .. FPS_SHOWN .. " FPS"
                    if FPS_SHOWN >= 60 then
                        GUI.titleLabel.TextColor3 = Color3.fromRGB(0,220,100)
                    elseif FPS_SHOWN >= 30 then
                        GUI.titleLabel.TextColor3 = Color3.fromRGB(255,180,0)
                    else
                        GUI.titleLabel.TextColor3 = Color3.fromRGB(255,60,60)
                    end
                end
            end
        end)

        -- FOV circle (independent of menu visibility)
        pcall(function()
        ...
        -- [FIX-IG4] Live auto-refresh ignore panel every 0.25s
        pcall(function()
            if ST.igOpen and GUI.igPanel and GUI.igPanel.Visible then
                local now = os.clock()
                if now - ST.igLastRefresh >= 0.25 then
                    ST.igLastRefresh = now
                    pcall(refreshIgnorePanel)
                end
            end
        end)

        -- ESP update (throttled ~5-10 hz)
        local nowT = os.clock()
                    ...
                    if pl ~= ME then
                        ...
                        if not S.ES.on then
                            ...
                            if c and al(c) then
                                ...
                                local rt  = fr(c)
                                if cam and rt and
                                (cam.CFrame.Position-rt.Position).Magnitude <= rngL then
                                    local esp = IESP[pl]
                                    if esp == nil then
                                    ...
                                    elseif esp.highlight then
                                        if esp.highlight.Parent==nil or esp.billboard.Parent==nil then
                                            IESP[pl] = nil
                                            local k = nowT
                                            if k-ST.killT < 8 then ST.kills=ST.kills+1 else ST.kills=1 end
                                            ST.killT = k
                                            ...
                                            if esp.txt2 then
                                                ...
                                                local dd  = safeFloor((cam.CFrame.Position-rt.Position).Magnitude,0)
                                                local hp  = "?"
                                                if hum then
                                                    local hv = hum.Health
                                                    if hv==hv and hv>=0 then hp=tostring(safeFloor(hv,0)) end
                                                end
                                                esp.txt2.Text = hp.." HP | "..dd.."m"
                                                esp.txt2.TextColor3 = hpColor(hum)
                                            end
...
print(string.rep("=", 60))
print("  V9.27 OPSYX  |  sUNC 100% / UNC 98-99%")
print("  [FIX-IG1] Ignore list sorted nearest-first by default")
print("  [FIX-IG2] Sort toggle button: NEAR <-> A-Z in panel")
print("  [FIX-IG3] Distance badge on every player row (color-coded)")
print("  [FIX-IG4] Auto-refresh every 0.25s while panel is open")
print("  [IMPROVE] Status bar: player count + ignored count")
print("  [IMPROVE] CLEAR button wipes all ignores at once")
print("  [IMPROVE] Row hover highlight, ClipsDescendants on list")
print("  Distance colors: green<50m  yellow<150m  red=far  gray=?")
print("  All V9.26 + V9.25 fixes retained")
print("  F1=Aimbot | F2=ESP | F3=Silent | F4=Trigger | F6=Wall")
print("  F5=KEYS  |  F7=Hide Menu (FOV stays ON)  |  RMB=Arm")
